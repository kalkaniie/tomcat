...
/* ------------------------------------------------------------ 
   ...
   ------------------------------------------------------------ */
	...
	function show() {
		...
		// 조회 날짜 출력
		printTable(0, this.width, '0', 0, 0, 0, '');
		printTr(0, "center");
			printTd(0, '', '', 7);
			printPrev(this.name);
			print('&nbsp;');
		...
		// 요일 출력
		printTable(0, this.width, '', 0, 0, 0, 'background-color:'+ this.linecolor +'; line-height:' + this.lineheight);
		...
		// 달력 textfield 출력
		printTable(0, this.width, '', 0, 0, 0, 'background-color:'+ this.bgcolor2 +'; line-height:' + this.lineheight);
		for (var i = 0; i < 42; i++) {
			printTr(0, 'center', 'background-color:'+ this.bgcolor2);
			for (var j=0; j < 7; j++) {
				
				printTd(0);
				printTextField( i, 1, 1, 'color:'+day_color[j] + '; cursor:hand; border-width:1px; border-color:'+this.tfCol+'; border-style:solid; background-color:'+this.tfCol+'; text-align:center', this.tfCol, this.focusCol);
				printTd();
				i++;
				if (i == 42 ) break;
			}
...
/* ------------------------------------------------------------ 
   ...
   ------------------------------------------------------------ */
	...
	// 일자 필드(gb: 0 ~ 41)
	function printTextField(gb, size, maxlength, style, tfCol, focusCol) {
		print ("<input type='text' name='tf" + gb + "' size='"+ size +"' maxlength='"+ maxlength + "' style='" + style + "' readonly tabindex=-1  onMouseOver=\"this.style.backgroundColor = '"+ focusCol+"';this.style.borderColor='"+ focusCol+"';\" onMouseOut=\"this.style.backgroundColor = '"+ tfCol+"';this.style.borderColor='"+ tfCol+"'\">");
	}
