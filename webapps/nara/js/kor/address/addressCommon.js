...
addressCommon_space.main = function() {
	function addressAddByMBox() {
		win_address_add = new Ext.Window({
			id:'kebi_ext_window',
			title:'편지함에 있는 주소추가',
			colsable:true,
			width:700, 
			...
			items:new Ext.Panel({
				...
				autoLoad:{
					url:'address.auth.do',
					params:{method:'address_mbox_list'},
					scripts:true
				}
			})
		});
	
		win_address_add.show();
	};
	...
	function mailAddreList() {
  		var objForm = win_address_add.getEl().dom.getElementsByTagName("form")[0] ;
  		var isChk = false;
		...
		if (!isChk) {
			Ext.Msg.alert('message', "선택된 편지함이 없습니다.");
			return ;
		}
	            ...
	            if(fp_up.getForm().isValid()){
		        	fp_up.getForm().submit({
		                ...
		                failure: function(fp_up, o){
		                	...
		                	Ext.Msg.alert('message', response.errors.msg)
		                }, 
		                success: function(fp_up, o){
		                	var response = Ext.util.JSON.decode(o.response.responseText);
		                	if (response.errors.msg == "") {
								Ext.Msg.alert('message','등록되었습니다')
		                	} else {
		                		Ext.Msg.alert('message','등록 실패')
		                	}
                        }
					});
				}
	...
	function warning() {
		Ext.Msg.alert('message', '양식의 칼럼 순서가 변경될 경우 정상적인 업로드가 되지 않습니다\n\n변경하지 마시고 필요정보만 입력 후 업로드하세요\n\n');
	};
	...
	function addressAdd() {
	  	var objForm = win_address_list.getEl().dom.getElementsByTagName("form")[0] ;
		
		if (!isCheckedCheckBox(objForm.ADDRESS_EMAIL)) {
			Ext.Msg.alert('message', "선택된 항목이 없습니다.");
			return ;
		} else {
