		...
		//object
		var targetIdentity ="";   //key값 
		var targetObjTitle="";	  //제목
		var targetObjId="";		  //실제key

		//tree
		var targetTreeIdentity  ="";
		var targetTree = "";
		
		//var jspPage = "/jsp/kor/webmail/test.jsp";
		var jspPage = "webmail.auth.do?method=ajax_move_mail";
...
ajax.dnd.SimpleDragSource.prototype = {
	...
	doMouseMove: function(e) {
		...
		if (!this.dragging) {
			this.dragging = true;
			ajax.GUI.setOpacity(this.element, 0.50);
		}
...
ajax.dnd.DropTarget.prototype = {
	...
	drop: function(dragSource) {
			...
			// this.element =>leftTree        element => item    

			targetTree =  this.element.getAttribute("id");
			targetTreeIdentity = this.element.getAttribute("id").substr(Tree.length,this.element.getAttribute("id").length);

			
			targetTreeId=document.getElementById(Tree + targetTreeIdentity +"id").innerHTML;

			var params = "MBOX_IDX="+encodeURIComponent(targetTreeId) + "&" + "M_IDX="+encodeURIComponent(targetObjId) ;
			if(flag == "true"){
				 doCommitToServer(jspPage, params);
			}
	}
}
...
ajax.dnd.DragSource.prototype = {
	...
	startDrag: function(event) {
		...
		//this.element.style.backgroundColor = "#ADC1D8";
		//this.element.style.width = "500px";
		//this.element.style.height = "20px";
		//this.element.style.left = ajax.Event.getMouseXY(event);
		ajax.GUI.setOpacity(this.element, 0.90);
	},
	isDragging: function() {
	...
	endDrag: function(event) {
		this.dragging = false;
		this.element.style.position = "absolute";
		//this.element.style.backgroundColor = "#ADCCF8";
		ajax.GUI.setOpacity(this.element, 1.0);

		 targetIdentity = this.element.getAttribute("id").substr(Obj.length,this.element.getAttribute("id").length);
		 targetObjTitle=document.getElementById(Obj + targetIdentity +"title").innerHTML;
		 targetObjId=document.getElementById(Obj+ targetIdentity +"id").innerHTML;




this.element.parentNode.removeChild(this.element);
		var isCrm= confirm( targetObjTitle   + ' 을(를) 이동하시겠습니까?');
