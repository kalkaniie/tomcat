function goMailList(view_type,mboxIdx,mboxName, panelName){
	var strUrl = "/mail/webmail.auth.do?method=mail_list_view&MBOX_IDX="+mboxIdx;
	if( view_type=="tag")
		strUrl = "/mail/webmail.auth.do?method=mail_list_view&VIEW_TYPE=tag&TAG_TYPE="+mboxIdx;
...
function goMailListReadOrNotRead(view_type,mboxIdx,mboxName, panelName,readOrNotRead){
	var strUrl = "/mail/webmail.auth.do?method=mail_list_view&MBOX_IDX="+mboxIdx+"&nMode="+readOrNotRead;
	if( view_type=="tag")
		strUrl = "/mail/webmail.auth.do?method=mail_list_view&VIEW_TYPE=tag&TAG_TYPE="+mboxIdx+"&nMode="+readOrNotRead;
	return goRightDivRender(strUrl, mboxName)
}

function goSendSms(strUrl, panelName, params){
	var leftUrl = "/jsp_std/kor/menu/leftsms.jsp";
	parent.document.getElementById("api").src =	leftUrl;
	goRightDivRenderParams(strUrl, params, panelName)
}

function goRightDivRender(strUrl, tabClassName,classId){
	if(document.getElementById("mainPanel") != null)
		document.getElementById("mainPanel").src =strUrl;
	else	
...
function goRightDivRenderParams(strUrl, params, tabClassName){
	if(document.getElementById("mainPanel") != null)
		document.getElementById("mainPanel").src = strUrl+params;
	else	
		parent.document.getElementById("mainPanel").src = strUrl+params;
}

function goParentRightDivRender(strUrl, tabClassName){
	opener.parent.document.getElementById("mainPanel").src =strUrl; 
}
...
function goLeftAndRightDivRender(strUrl, tabClassName, menuSubName){
	...
	if(parent.document.getElementById("api") != null){
		...
		document.getElementById("api").src =leftUrl;
		document.getElementById("mainPanel").src =strUrl;
	}	
...
function goMailPrewView(m_idx,actionPage){
	...
	if( !(mail_preview_win instanceof Ext.Window)){
		mail_preview_win = new Ext.Window({
			...
			border:false,
			modal:false,
			autoLoad: {url: '/mail/webmail.auth.do?method=mail_view_std_preview&callPage='+actionPage+'&M_IDX='+m_idx,scripts:true,scope: this},			
			tools : [{id:'close',
		...
		mail_preview_win.show();
			...
			url: '/mail/webmail.auth.do?method=mail_view_std_preview&callPage='+actionPage+'&M_IDX='+m_idx,
			scripts: true
		});		
	}
}
...
function goBbsPrewView(bbs_idx, b_idx, actionPage){
	if( !(bbs_preview_win instanceof Ext.Window)){
		bbs_preview_win = new Ext.Window({
			...
			bodyBorder :false,
			border:false,
			autoLoad: {url: "/mail/board.auth.do?method=mypage_bbs_preview&BBS_TYPE=1&BBS_IDX="+bbs_idx+"&B_IDX="+b_idx+"&callPage="+actionPage,scripts:true,scope: this},
			tools : [{id:'close',
		        	handler: function(e, target, panel){
...
function goMailWrite(strUrl){
	goRightDivRender(encodeURI("/mail/webmail.auth.do?method=mail_write&"+strUrl),"편지쓰기");
}

function goOpWinFromMailWrite(strUrl){
	goParentRightDivRender("/mail/webmail.auth.do?method=mail_write&"+strUrl,"편지쓰기");
	self.close();
}
...
function getChildNodeElement(nodeId){
	for( i=0; i<mainPanel.body.dom.childNodes.length; i++){
		if( mainPanel.body.dom.childNodes[i].id == nodeId){
			return mainPanel.body.dom.childNodes[i];
		}
	}
}
...
//		if(msgOk) {
			//parent.document.getElementById("kebi_footer_message").innerHTML=alertMessage;
			alert(alertMessage)
//		}
		return true;
//	}else{
...
function tagWinOpen(){
	if( !(tag_win instanceof Ext.Window)){
		tag_win = new Ext.Window({
			...
			bodyBorder :false,
			border:false,
			autoLoad: {url: '/jsp_std/kor/webmail/tag_info.jsp',scripts:true,scope: this},
			tools : [{id:'close',
		        	handler: function(e, target, panel){
...
function showLoadingMessage(str){
	var parent1= parent;
	//alert(parent.document.getElementById('loading_message'))
	//alert(parent.contentWindow.document.getElementById('loading_message'))
	//parent.contentWindow.document.getElementById('loading_message').style.visibility="visible";
	//parent.contentWindow.document.getElementById('loading_message_txt').innerHTML= str;
}

function unLoadingMessage(){
	try{
	//parent.contentWindow.document.getElementById('loading_message_txt').innerHTML= '';
	//parent.contentWindow.document.getElementById('loading_message').style.visibility='hidden';
	}catch(e){}
}	
...
function deleteMailInMbox(_MBOX_TYPE, _MBOX_IDX){
	...
	var boxName = "";
	if(_MBOX_TYPE =="3") boxName= "휴지통"; else boxName= "광고편지함";
   	var action_msg = boxName + "의 모든 메일 삭제되어 복구가 불가능해집니다.\n"+boxName + "을 비우시겠습니까?";
   	...
   	Ext.Ajax.request({
		...
		success : function(response, options) {
    		...
    		if (resultXML.records[0].data.RESULT == "success") {
				...
				try{											
					if(parent.document.getElementById("mainPanel").src.indexOf("/mail/webmail.auth.do?method=mail_list_std") != -1){
						//parent.document.getElementById("mainPanel").contentWindow.location.reload();
						parent.document.getElementById("mainPanel").src = parent.document.getElementById("mainPanel").src;
					}
				}catch(e){}
...
function goLeftTreeMailList(mboxIdx,mboxName, mbox_public ,panelName){
	if(mbox_public == 'S'){
		window.open('/mail/mbox.auth.do?method=webmail_idpwd&MBOX_IDX='+mboxIdx+ '&mboxName='+mboxName+ '&panelName='+panelName , 'secure_check', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=auto,resizable=yes,width=300,height=105');
		return ;
	}
		
	var strUrl = "/mail/webmail.auth.do?method=mail_list_std&VIEW_TYPE=normal&MBOX_IDX="+mboxIdx;
	return goRightDivRender(strUrl, mboxName)
}
...
function searchFormByActiveTab(formName){
	try{
		var objForms = parent.document.getElementById("mainPanel").dom.getElementsByTagName('form');
		var returnForm;
		for(var ii=0; ii<objForms.length; ii++) {
...
function goSendToMeMbox(){
	var strUrl = "/mail/webmail.auth.do?method=mail_list_view&toMe=Y";
...
function goActiveTabMyPage(){
	...
	parent.document.getElementById("mainPanel").src ="/jsp_std/kor/user/user_mypage.jsp";
}
...
function searchFormByAllTab(formName){
	try{
		var objForms = parent.document.getElementById("mainPanel").getElementsByTagName('form');
		var returnForm;
		for(var ii=0; ii<objForms.length; ii++) {
			if(objForms[ii].name.indexOf(formName) ==0){
				returnForm = objForms[ii];
...
function getDocbodyHeight(){
 return parent.document.getElementById("doc-body").clientHeight;
}
...
function callAjaxFailure(response, options) {
	...
	var url = encodeURI("/mail/message.public.do?method=aj_message&jdf_user_msg=" + jdf_user_msg + "&jdf_debug_msg=" + jdf_debug_msg + "&jdf_error_code=" + jdf_error_code);

	if (jdf_error_code != null && jdf_error_code == "S008") {
		
		
		if (parent.document.getElementById("mainPanel") != null) {
			
			parent.document.getElementById("mainPanel").src = url;
			
//			mainPanel.body.load({
//				url: 'message.public.do', 
//				scripts:true, 
//				scope: this,
//				params:{
//					method:'aj_message',
//					jdf_user_msg: jdf_user_msg,
//					jdf_debug_msg: jdf_debug_msg,
//					jdf_error_code: jdf_error_code
//				}
//			});
		} else {
			location.href = url;
...
function stripEMailAddr(string) {
   var objStrip = new RegExp();
   objStrip = /[<][^>]*[>]/gi;
   var str = (string.match(objStrip, "")+"").replace("<", "").replace(">", "");
   return str.substring(0, str.indexOf("@"));
}

function stripHTMLtagReverse(string) {
   var objStrip = new RegExp();
   objStrip = /[<][^>]*[>]/gi;
   return objStrip;
}
function stripQuotation(string,patt) {
   var objStrip = new RegExp();
   objStrip = /["]/gi;
