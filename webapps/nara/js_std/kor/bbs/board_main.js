...
boardMainSpace.board = function() {
					...
					if (b_attach.length > 0) {
						...
						for(var i=0; i<arr.length; i++) {
							try {
								...
								if(arr[i].length > 0)
									attachStr += "<a href='javascript:;' style='margin-left:3px;'><img src=/images/kor/ico/" + FileType[1] + " onClick=\"fileDownload('"+bbs_idx+"','"+b_idx+"','"+i+"','"+arr[i]+"')\" alt=\""+arr[i]+"\" style='_padding-bottom:1px'></a>";
							}catch(e) {}
						}
						attachStr +="</em>"
					}
    ...
    return {
    	...
    	init : function() {
			...
			Ext.EventManager.onWindowResize(function(){Ext.getCmp('id_board_main').setWidth(Ext.get(document.getElementById("doc-body")).getWidth())}, Ext.getCmp('id_board_main'), true);		
   			Ext.EventManager.onWindowResize(function(){Ext.getCmp('id_board_main').setHeight(Ext.get(document.getElementById("doc-body")).getHeight()-60)}, Ext.getCmp('id_board_main'), true);
    	}
    }
}();
