...
Ext.ux.HTMLEditorToolbar = Ext.extend(Ext.Toolbar, {
  ...
  addTools: function(tools) {   // add tools (pre-render)
    tools = (tools instanceof Array) ? tools : [tools];
    for (var i = 0, len = tools.length; i < len; i++) {
      this.tools.add(tools[i]);
    }
  },
  insertTools: function(index, tools) {  // insert tools (pre-render)
    tools = (tools instanceof Array) ? tools : [tools];
    for (var i = 0, len = tools.length; i < len; i++) {
      this.tools.insert(index + i, tools[i]);
    }
  },
  insertToolsBefore: function(itemId, tools) {// insert tools before another tool (pre-render)
    var index = this.tools.indexOfKey(itemId);
    this.insertTools(index, tools);
  },
  insertToolsAfter: function(itemId, tools) {// insert tools after another tool (pre-render)
    var index = this.tools.indexOfKey(itemId) + 1;
    this.insertTools(index, tools);
  },
  renderTool: function(tool) {// render tools (performed after tools/plugins have been configured/reordered)
  ...
  onRender: function(ct, position) {// overrides Ext.Toolbar.onRender
...
Ext.ux.HTMLEditor = Ext.extend(Ext.form.HtmlEditor, {
  fontFamilies:['굴림','굴림체','돋움','돋움체','궁서','궁서체','Arial','Courier New','Tahoma','Times New Roman','Verdana'],
  defaultFont:'굴림',
  toolbarItems: [
    'fonts',
  	'allformats',
  ...
  toolbarItemExcludes: [], // as an alternative, the toolbarItemExcludes list can be used to exclude items from the toolbarItem list
  initComponent: function() {  // overrides Ext.form.HtmlEditor.initComponent first function to be called upon creation of the editor
  ...
  createFontOptions: function() {// overrides Ext.form.HtmlEditor.createFontOptions
  ...
  btn: function(id, toggle, queryState, handler) { // create default button config
    return {
      itemId: id,
  ...
  onEditorEvent: function(e) { // overrides Ext.form.HtmlEditor.onEditorEvent
