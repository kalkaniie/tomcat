...
left_note_space.left_note = function() {
	function goNoteList(_idx) {
		var strUrl = "/mail/note.auth.do?method=showMain_std&NOTE_BOX_TYPE=" + _idx;
		parent.document.getElementById("mainPanel").src =strUrl;
	};
	
	function goNoteReconfList() {
		var strUrl = "/mail/note.auth.do?method=note_reconf_list_std";
		parent.document.getElementById("mainPanel").src =strUrl;
	};
...
//					autoSize:true,
//					autoLoad:{
//						url:'/mail/note.auth.do?method=note_regist_form',
//						params:{
//							NOTE_TO:_note_to,
//							NOTE_IDX:_note_idx
//						},
...
//		win_note_write.show();	
		 ...
		 var t_width =566;
		 var t_height = 420;
		 var env_val = "status=yes,toolbar=no,scrollbars=no,width="+t_width+",height="+t_height;
		MM_openBrWindow('/mail/note.auth.do?method=note_regist_form','note_write',env_val);
	};
	...
	function popUserGroupTreeByNote() {
		win_organ_tree = new Ext.Window({
			...
			items:new Ext.Panel({
				...
				autoSize:true,
				autoLoad:{
					url:'/mail/note.auth.do?method=usergroup_tree',
					scripts:true
				}
			})
		});
	...
	function renderUserGroupTree(_userGroupIdx, _userGroupName) {
	    var tree = new Ext.tree.TreePanel({
	        ...
	        loader: new Ext.tree.TreeLoader({ dataUrl:'/mail/note.auth.do?method=aj_getUserGroup'})
	    });
			  		...
			  		if (resultXML.records[0].data.RESULT == "success") {
			  			...
			  			if(duplResultXML.records.length > 0){
							var TARGET = new Array(), USERS_IDX = new Array(), USERS_NAME = new Array(), USERS_DEPARTMENT = new Array();
			  				...
			  				var link_str = "/mail/webmail.auth.do?method=getLocalUser";
...
left_note_space.note_detail = function() {
	...
	function download(_note_attache_idx){
		...
		for(var ii=0; ii<objForms.length; ii++) {
	  	...
	  	objForm.action="/mail/note.auth.do";
