  ...
  create: function( fromBuddyId , fromBuddyNm , sentTime ,fileCount, fileSize ,fileInfo, noteMessage ,fontInfo) {
    ...
    if( fileInfo.length > 0 && fileCount > 0)
    {
    	for(var i=0;i<tmpfileInfo.length;i++){
	    	...
	    	viewFileInfo =viewFileInfo +'<img src="/jsp/kor/realtime/images/default/tree/leaf.gif" absmiddle>&nbsp;'
	            	+'<a href="http://www.naravision.net/messenger/notefiledwn?file1='+filewebpath+'&file2='
    ...
    var form = new Ext.form.FormPanel({
        ...
        items: [{
            ...
            xtype:'panel',
            id: 'fileProgress',
            html: viewFileInfo /*'<img src="/jsp/kor/realtime/images/default/tree/leaf.gif" absmiddle>&nbsp;'
            	+'<a href="http://www.naravision.net/realtime/notefiledwn?file1='+filewebpath+'&file2='
     ...
     if(Note_popup){
	       ...
	       var f_panel = new Ext.Panel(
	        		{width:395,height:150,
	        		 ...
	        		 autoLoad: {url: '/jsp/kor/realtime/flex_loading.jsp?width=150'
	        		, scripts:true
	        		, scope: this}
	        		})
	        ...
	        baseCls: 'x-plain',
	        labelWidth: 55,
	        url: 'http://www.naravision.net/jsp/kor/realtime/flex_file_save.jsp',
	        defaultType: 'textfield',
	        fileUpload : true,
	        items: [
	        {
	            fieldLabel: '받는사람',
	            name: buddyIndex + '_to',
	            value: toBuddyNm + "(" + hidef.receive_user_id  + ")" ,
	            readOnly : true,
	            anchor: '100%'  // anchor width by percentage
	        },f_panel
	        ,{
...
//	        },
	 	...
	 	var f_panel = new Ext.Panel(
	        		{width:395,height:110,
	        		 //autoLoad: {url: '/flex/jsp/msg_fileupload_flex.jsp'
	        		 autoLoad: {url: '/jsp/kor/realtime/flex_loading.jsp'
	        		, scripts:true
	        		, scope: this}
	        		})
        ...
        baseCls: 'x-plain',
        labelWidth: 55,
        url: 'http://www.naravision.net/jsp/kor/realtime/flex_file_save.jsp?width=80',
        defaultType: 'textfield',
        fileUpload : true,
        items: [
        {
...
function js_Note_send(win,form)
{
   ...
   // alert(form.getForm().findField(buddyIndex + '_content').getValue());
  // alert(form.getForm().findField(buddyIndex + '_content').cleanHtml(form.getForm().findField(buddyIndex + '_content').getValue()));
...
function js_Note_Success(buddyIndex, fromBuddyId, fromBuddyNm , toBuddyId ,noteMessage,fileCount,fileSize,fileInfo,webpath ,fontInfo)
{
    ...
    // alert(webpath + ":dddddddddd" + fileInfo);
    //alert(hidef.real_user_id);
...
function js_notify_note(obj)
{
    ...
    // alert(fromBuddyId + ":"+ fromBuddyNm + ":"+ fn_getDateFormatString(sentTime, 'YYYY년 MM월 DD일 hh:mm:ss' ) + ":"+fileCount + ":"+fileSize + ":"+fileInfo + ":"+noteMessage );
...
var multiUserNoteWindow = {
    create : function(originalWin , originalForm , toBuddyId,toBuddyNm ,fromBuddyId,buddyIndex) {
            ...
            var win = new Ext.Window({
              ...
              width:280,
              height:280,
              //id: "invite_MlMsg_window_" + szIndex,
              autoScroll:true,
              resizable:false,
              plain: true,              
              bbar: [
