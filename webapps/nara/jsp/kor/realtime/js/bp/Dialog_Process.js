...
{   
	alert(szIndex +":"+ szDlgIndex+":"+szSendID+":"+szRecvID);
	if(szDlgIndex == -1)
	{
...
function processMakeDialogue(szIndex, szDlgIndex, szSendID, szRecvID)
{   
	//alert(szIndex +":"+ szDlgIndex+":"+szSendID+":"+szRecvID + ":" + intervalDay_flag);
	// --realTime 시간 경과 되었으면 자동
	if(szDlgIndex == -1 || intervalDay_flag == 'true')
	{
			//alert(1);
			//updateBuddy(szRecvID, '-1', '');
			//alert(2);
			//alert(intervalDay_flag);
			//추가 ialogIndex 값이.. "-1" 이 넘어오면 이사람과는 대화를 할수 없는 상대로 간주하고 쪽지창으로 띄워줍다..
			
			parent.js_Note_open(szIndex);
			//alert(3);
			return;
	}
	...
	{	
	 	...
	 	//js_curtUserAdd(szDlgIndex, buddyinfo[7]);
	 	//js_curtUserAdd(szDlgIndex, receive_user_id);
	 	js_curtUserAdd(szDlgIndex, szSendID);
		js_curtUserAdd(szDlgIndex, szRecvID);
		
	    parent.makeDialogue(szDlgIndex, receive_user_nm, szRecvID);
...
//        var isUnderline = (mlMsg_textarea.style.textDecoration == 'underline' ? 'true' : 'false');
		...
		//alert(fontColor + ":" + fontColor.substring(1,3)+ ":" + fontColor.substring(3,5)+ ":" + fontColor.substring(5,7));
		...
		mlMsg_ctnt.innerHTML = mlMsg_ctnt.innerHTML + "<b>"+user_nm+":</b><br/>"+ trnsMsg;
       ...
       // alert(receive_user_id);
		//alert(dialogueID + ";"+ uaconfArr[3] + ";"+ uaconfArr[1])
        //DialogSend(dialogueID,uaconfArr[3],uaconfArr[1],setFont,recvusers, msg.stripTags().stripScripts());
        //DialogSend(dialogueID,"test1_1234@nwc.demo.kebi.com","테스트",setFont,"test2@demo.kebi.com", msg.stripTags().stripScripts(), "test1@demo.kebi.com");
        DialogSend(dialogueID,user_id,user_nm,setFont,receive_user_id, msg.stripTags().stripScripts(), real_user_id);
	}
}
...
{       
	  //  alert(szIndex +":"+ szSendID +":"+ szSendName +":"+  szFont +":"+ szDate +":"+ szCont +":"+ szDisplayID)
        var msg = '';
...
function js_room_leave_relay(szIndex, szSendID, szSendName)
{
   ...
   // alert(curtUserCnt + "LLLeeeLLLLLLLl");
   // js_curtUserDisplay(szIndex) //대화창에 참여한 사람들 display
	// alert(curtUserCnt + "LLLLLLLLLLl");
   if (curtUserCnt > 1) {
        szSendName = szSendID;
        szCont = "<font color='red'>" + szSendName + "님께서  대화를 종료하셨습니다.</font><br/>";
        js_alertMsg(szIndex, szCont) // 메시지 출력
        
       // alert(parent.$('msg_'+szIndex).disabled);
        parent.$('msg_'+szIndex).style.backgroundColor='#C0C0C0';
        parent.$('msg_'+szIndex).disabled = true;
...
function js_curtUserDisplay(szIndex) {
    ...
    /*
    */
  // alert(arrCurtUsers.size());
    if(arrCurtUsers.size() > 0) {
...
function js_alertMsg(szIndex, szCont) {
    ...
    parent.$('mlMsg_window_cntnt_'+szIndex).innerHTML = parent.$('mlMsg_window_cntnt_'+szIndex).innerHTML + szCont;
  	...
  	parent.$('mlMsg_window_cntnt_'+szIndex).scrollTop = parent.$('mlMsg_window_cntnt_'+szIndex).scrollTop + 600;
    // 창 화면 밖으로 일때 메세지 전송
	if(!Object.isUndefined(parent.openPopDialHash.get(szIndex))) {
...
// Font 처리 
function js_msgStyeTras(msg, isBold, isItalic, isUnderline, fontName, fontSize, fontColor) {
//function js_msgStyeTras(msg,fontName, fontSize, fontColor, isBold, isItalic, isUnderline) {
//    msg = (isBold == 'true' ? "<b>" : "") + (isItalic == 'true' ? "<i>" : "") + (isUnderline == 'true' ? "<u>" : "") + "<span style=\"font-family:" + fontName + "; font-size:" + fontSize + "pt; color:" + fontColor + ";\">" + msg + "</span>" + (isBold == 'true' ? "</b>" : "") + (isItalic == 'true' ? "</i>" : "") + (isUnderline == 'true' ? "</u>" : "") + "<br/>\n";
	 msg = (isBold == '1' ? "<b>" : "") + (isItalic == '1' ? "<i>" : "") + (isUnderline == '1' ? "<u>" : "") + "<span style=\"font-family:" + fontName + "; font-size:" + fontSize + "pt; color:" + fontColor + ";\">" + msg + "</span>" 
     + (isUnderline == '1' ? "</u>" : "")+ (isItalic == '1' ? "</i>" : "") + (isBold == '1' ? "</b>" : "") +   "<br/>\n";
    return msg;
...
function js_saveMsg(dialogueID, recvid) {
    ...
    var recv_id = buddyinfo[7];
    var recv_user_nm = buddyinfo[3];
