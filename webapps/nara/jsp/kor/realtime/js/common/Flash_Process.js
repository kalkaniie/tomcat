...
function receiveMsg(op_code,resultmsg)
{
   ...
   {      
      alert("Flash Response ERROR OPCODE["+ op_code + "]" + resultmsg);
   }
}
...
{       
	 //alert("send  =>" + op_code + "<====>" +message + ":   " + Login_Flag);
	 
    if((Login_Flag == 1)||(op_code == UA_LOGON_REQ)||(op_code == UA_INVITE_REQ)
    	||(op_code == UA_MAIL_LOGON_REQ||(op_code == UA_ROOM_LEAVE_NOTI)))
    {
      	try{
      		//alert(op_code);
            var send=new Object();
...
{	
   ...
   //alert("Record_arr:"+ Record_arr);
	...
	//cho added	
	setUAConfigHTML(uaconfArr);
	window.setTimeout("AppleWorkRequest('30003101',user_id);", 100);
	//js_getReqBuddyList();
...
function keepAlive_packet()
{
	var op_code = UA_KEEPALIVE_DUMMY_REQ ;
	var strSendData = "";
	strSendData = user_id;
...
function realTimeStart(){
	 //alert(user_id);
	DialogIndexReq(user_id, receive_user_id);
}
