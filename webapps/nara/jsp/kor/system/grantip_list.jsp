...
function condSearch() {
	...
	objForm.action = "grantip.system.do?method=grantIp_list";
	// objForm.submit();
	objForm.submit();
}

function goGrantIpUpdate(idx,ipStr) {
	var link="grantip.system.do?method=update_form&grantip_idx="+idx+"&grantip="+ipStr;
	MM_openBrWindow(link ,"ip_insert","status=no,toolbar=no,scrollbars=yes,resizable=yes,width=350,height=90");
}

function regist() {
	var link="grantip.system.do?method=regist_form";
  	MM_openBrWindow(link ,"ip_insert","status=no,toolbar=no,scrollbars=yes,resizable=yes,width=350,height=90");
}
...
function removeGrantIp() {
	...
	if(isRemove){
	    objForm.action="grantip.system.do?method=delete_grantip";
	    document.f.submit();;
	}
}
//-->
...
<div class="k_puTit">
<h2 class="k_puTit_ico2">시스템관리 <strong>관리자접근제어관리</strong></h2>
<hr />
</div>
<ul class="k_tip_ul">
