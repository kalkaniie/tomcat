...
<jsp:useBean id="signEntity" class="com.nara.jdf.db.entity.SignEntity" scope="request" />
<%
...
<script languate="javascript">
  ...
  var imgTool=true, letterTool=false, formletterTool= false;
  function updateSign() {
	...
	objForm.action = "signstmt.auth.do?method=sign_update";
	objForm.submit(); 
  }
  ...
  function preView() {
	var objForm = document.user_sign;
  	...
  	MM_openBrWindow(link,"preWin","width=700,height=400");
  }
...
<input type=hidden name='SIGN_IDX' value='<%= signEntity.SIGN_IDX%>'>
<TEXTAREA id=temp_content style="display:none;"><%=signEntity.SIGN_STMT.trim()%></TEXTAREA>
...
<table class="k_tb_other" style="border-bottom:none">     
  <tr>
    <th width="50">제목</th>
    <td>
    	<input type="text" name="SIGN_TITLE" value="<%= signEntity.SIGN_TITLE%>" class="intx00" maxlength="20" style="width:230px"/>
    	&nbsp;&nbsp;&nbsp;<input type=checkbox name=SIGN_DEFAULT value='1' <%=signEntity.SIGN_DEFAULT ==1 ? "checked" : ""%>/>기본서명
...
<p class="k_fltR" style="padding:10px 5px 10px">
  <a href="javascript:updateSign();"><img src="/images/kor/btn/btnA_modify.gif" /></a>
