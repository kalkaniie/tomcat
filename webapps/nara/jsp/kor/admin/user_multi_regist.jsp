...
<%@page import="com.nara.jdf.db.entity.UserRegistEntity"%>
...
function registFile() {
	...
	objForm.action = "user.admin.do?method=user_list_attach";
	objForm.submit();
}

//선택한리스트삭제
function selectedDelete() {
 	var objForm = document.multi_regist;
  	if(!isCheckedOfBox(objForm, "USERS_REGIST_IDX")){
    	alert("삭제할 사용자 정보를 선택하십시오.");
    	return;
  	}
  	    
  	if(confirm("선택하신 사용자 정보가 삭제됩니다.\n삭제하시겠습니까?")){
    	objForm.method.value = "remove_multi_user";
...
function allRegist() {
	var objForm = document.multi_regist;
  	...
  	if(confirm("전체 사용자 리스트가 가입됩니다. \n가입하시겠습니까?")){
    	objForm.method.value = "regist_multi_user";
    	objForm.action="user.admin.do";
    	objForm.submit();
  	}
}
...
function keywordSrch() {
	...
	objForm.action = "user.admin.do?method=user_multi_regist_form";
	objForm.submit();
}
//-->
</script>

<form name="multi_regist" method="post">
...
<ul class="k_tip_ul">
	...
	<li>사용자 리스트를 아래의 형식에 맞추어 등록한 후 일괄적으로 등록 처리합니다.</li>
</ul>
<div class="k_puAdmin">
<ul class="k_tab_menu">
	...
	<li class="k_tab_menuOn"><b><a href="user.admin.do?method=user_multi_regist_form">일괄등록</a></b></li>
...
<table class="k_tb_other4">
	<tr>
		<th width="20" scope="col"><img src="/images/kor/ico/ico_check.gif" onClick="checkAll(document.multi_regist, 'USERS_REGIST_IDX');" /></th>
...
%>
	...
	<%
		...
		while(iterator.hasNext()) {
			UserRegistEntity entity = (UserRegistEntity)iterator.next();			
%>
	<tr>
		<td><input type="checkbox" name="USERS_REGIST_IDX" id="checkbox" value="<%=entity.USERS_REGIST_IDX %>" /></td>
		<td><%=entity.USERS_REGIST_ID %></td>
		<td class="k_txAliC"><a href="javascript:MM_openBrWindow('admin103_pop.html','','scrollbars=yes,resizable=yes,width=740,height=600');"><%=entity.USERS_REGIST_NAME %></a></td>
		<td class="k_txAliC"><%=entity.USERS_REGIST_JUMIN1%>-<%=entity.USERS_REGIST_JUMIN2%></td>
		<td class="k_txAliC"><%=entity.USERS_REGIST_DEPARTMENT %></td>
		<td class="k_txAliC"><%=entity.USERS_REGIST_LICENCENUM %></td>
		<td class="k_txAliC"><%=entity.USERS_REGIST_PASSWD%></td>
	</tr>
	<%
		}
	}
...
<a href="javascript:allRegist();"><img src="/images/kor/btn/popup_batchProc.gif" /></a></span></p>
...
<div class="k_puAdmin_sBox"><select name="strIndex">
	<option value="USERS_REGIST_NAME">이름</option>
	<option value="USERS_REGIST_ID">아이디</option>
	<option value="USERS_REGIST_JUMIN">주민등록번호</option>
	<% if(domainEntity.DOMAIN_TYPE.equals("C")) { %>
	<option value="USERS_REGIST_DEPARTMENT">부서명</option>
	<option value="USERS_REGIST_LICENCENUM">사번</option>
	<% } else { %>
	<option value="USERS_REGIST_DEPARTMENT">학과</option>
	<option value="USERS_REGIST_LICENCENUM">학번</option>
	<% } %>
</select> 
...
<script language=javascript>
setSelectedIndexByValue( document.multi_regist.strIndex, "<%=strIndex%>" );
</script>
