...
<%@page import="com.nara.jdf.db.entity.UserPauseEntity"%>
...
function registFile() {
	...
	objForm.action = "user.admin.do?method=user_list_attach_pause";
	objForm.submit();
}

//선택한리스트삭제
function selectedDelete() {
 	var objForm = document.multi_pause;
  	if(!isCheckedOfBox(objForm, "USERS_PAUSE_IDX")){
    	alert("삭제할 사용자 정보를 선택하십시오.");
    	return;
  	}
  	    
  	if(confirm("선택하신 사용자 정보가 삭제됩니다.\n삭제하시겠습니까?")){
    	objForm.method.value = "remove_multi_user_pause";
...
function allRegist() {
	var objForm = document.multi_pause;
  	...
  	if(confirm("전체 사용자 리스트가 중지됩니다. \n중지하시겠습니까?")){
    	objForm.method.value = "pause_multi_user";
    	objForm.action="user.admin.do";
    	objForm.submit();
  	}
}
...
function keywordSrch() {
	...
	objForm.action = "user.admin.do?method=user_multi_pause_form";
	objForm.submit();
}
//-->
</script>

<form name="multi_pause" method="post">
...
<ul class="k_tab_menu">
	...
	<li class="k_tab_menuOn"><b><a href="user.admin.do?method=user_multi_pause_form">일괄중지</a></b></li>
...
<table class="k_tb_other4">
	<tr>
		<th width="20" scope="col"><img src="/images/kor/ico/ico_check.gif" onClick="checkAll(document.multi_pause, 'USERS_PAUSE_IDX');" /></th>
...
%>
	...
	<%
		...
		while(iterator.hasNext()) {
			UserPauseEntity entity = (UserPauseEntity)iterator.next();			
%>
	<tr>
		<td><input type="checkbox" name="USERS_PAUSE_IDX" id="checkbox" value="<%=entity.USERS_PAUSE_IDX %>" /></td>
		<td><%=entity.USERS_PAUSE_ID %></td>
		<td class="k_txAliC"><a href="javascript:MM_openBrWindow('admin103_pop.html','','scrollbars=yes,resizable=yes,width=740,height=600');"><%=entity.USERS_PAUSE_NAME %></a></td>
		<td class="k_txAliC"><%=entity.USERS_PAUSE_JUMIN1%>-<%=entity.USERS_PAUSE_JUMIN2%></td>
		<td class="k_txAliC"><%=entity.USERS_PAUSE_DEPARTMENT %></td>
		<td class="k_txAliC"><%=entity.USERS_PAUSE_LICENCENUM %></td>
		<td class="k_txAliC"><%=entity.USERS_PAUSE_PASSWD%></td>
	</tr>
	<%
		}
	}
...
<a href="javascript:allRegist();"><img src="/images/kor/btn/popup_userPause.gif" /></a></span></p>
...
<div class="k_puAdmin_sBox"><select name="strIndex">
	<option value="USERS_PAUSE_NAME">이름</option>
	<option value="USERS_PAUSE_ID">아이디</option>
	<option value="USERS_PAUSE_JUMIN">주민등록번호</option>
	<% if(domainEntity.DOMAIN_TYPE.equals("C")) { %>
	<option value="USERS_PAUSE_DEPARTMENT">부서명</option>
	<option value="USERS_PAUSE_LICENCENUM">사번</option>
	<% } else { %>
	<option value="USERS_PAUSE_DEPARTMENT">학과</option>
	<option value="USERS_PAUSE_LICENCENUM">학번</option>
	<% } %>
</select> 
...
<script language=javascript>
setSelectedIndexByValue( document.multi_pause.strIndex, "<%=strIndex%>" );
</script>
