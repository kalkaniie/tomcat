...
<div class="k_content" style="height: 100%; position: relative">
<div class="k_functionA">
<p class="k_functionLeftA">
<%	if (isWriteAuth && bbsEntity.BBS_USE_REPLY == 1){ %> 
<a href="javascript:board_detail_space.board_detail.reply_article('<%=boardEntity.BBS_IDX %>', '<%=boardEntity.B_IDX %>');">
<img src="/images/kor/btn/btnA_BreplyAarticle.gif" /></a> <%	} %> <%	if(BbsAuthService.isArticleAuth(bbsEntity, userEntity, boardEntity.USERS_IDX)){%>
<a href="javascript:board_detail_space.board_detail.modify_article('<%=boardEntity.BBS_IDX %>', '<%=boardEntity.B_IDX %>')">
<img src="/images/kor/btn/btnA_Bmodify.gif" /></a> 
<a href="javascript:board_detail_space.board_detail.removeArticle();">
<img src="/images/kor/btn/btnA_Bdelete.gif" /></a> 
<% } %> 
<a href="javascript:board_detail_space.board_detail.printPage('<%=boardEntity.B_IDX %>')">
<img src="/images/kor/btn/btnA_Bprint.gif" /></a> 
<%	
	if(BbsAuthService.isAdminAuth(userEntity, bbsEntity, organizeEntity)){
   		if(isTop.equals("1")){
%> 
<a href="javascript:board_detail_space.board_detail.topArticleSetting('<%=boardEntity.BBS_IDX %>', '<%=bbsEntity.BBS_TYPE %>', '<%=boardEntity.B_IDX %>', '0');">
<img src="/images/kor/btn/btnA_BtopArticleNone.gif" /></a> 
<%
   		} else {
%> 
<a href="javascript:board_detail_space.board_detail.topArticleSetting('<%=boardEntity.BBS_IDX %>', '<%=bbsEntity.BBS_TYPE %>', '<%=boardEntity.B_IDX %>', '1');">
<img src="/images/kor/btn/btnA_BtopArticleSett.gif" /></a> 
<%   			
   		}
	}
   %>
</p>
<p class="k_functionRightA"><img src="/images/kor/ico/ico_list.gif" /><a	href="javascript:board_detail_space.board_detail.board_list('<%=bbsEntity.BBS_IDX %>', '<%=bbsEntity.BBS_TYPE %>');">목록</a>&nbsp;&nbsp;
<% if(Integer.parseInt(B_IDX_PRE) != 0){ %> 
<img src="/images/kor/ico/ico_listUp.gif" /> 
<a href="javascript:board_detail_space.board_detail.pageMoveArticle('<%=boardEntity.BBS_IDX %>', '<%=B_IDX_PRE %>');">윗글</a>&nbsp;&nbsp;
<% } else { %> 
...
<p class="k_grBlink"><b class="k_gridB_tit"><img src="/images/kor/bullet/arrow_tit.gif" /> 
...
<table>
	...
	<tr>
		...
		<td class="k_tdSt2"><a href="javascript:board_detail_space.board_detail.mail_write('<%=boardEntity.USERS_IDX %>', '<%=boardEntity.B_NAME %>');"><%=boardEntity.B_NAME %>(<%=boardEntity.USERS_IDX %>)</a>
		</td>
	</tr>
</table>
</div>
...
%>
	<li><img src="/images/kor/ico/<%=arrayFiletype[1]%>" /><a href="javascript:board_detail_space.board_detail.download(<%=i%>,'<%=attache[i].replaceAll("'","")%>');"><%=com.nara.jdf.jsp.HtmlUtility.translate(attache[i])%></a>
	<%
				if(f.length() < 1048576) {
    				out.println("<i>("+df.format((double)f.length()/1024)+"KB)</i></li>");
				} else {
    				out.println("<i>("+df.format((double)f.length()/1048576)+"MB)</i></li>");
  				}
			}
		}
%>
...
<div class="k_txView" style="overflow: auto" id="b_content<%=uniqStr%>">
<iframe src="/jsp/kor/bbs/board_detail_iframe.jsp?uniqStr=<%=uniqStr%>" id="board_detail_iframe<%=uniqStr%>" name="board_detail_iframe<%=uniqStr%>" height="100%" width="100%" frameborder="no" scrolling="auto" style="border:0px; padding:0; margin:0;"></iframe>
</div>

<div>
<div class="k_boxSt" id="tb_reply<%=uniqStr %>">
<div class="k_boxSt_t">
...
%>
	...
	<tr id="tr_reply_<%=appendEntity.APPEND_IDX %>">
		...
		<td width="150"><b><%=com.nara.jdf.jsp.HtmlUtility.translate(appendEntity.APPEND_NAME)%>(<%=appendEntity.USERS_IDX.substring(0,appendEntity.USERS_IDX.indexOf("@"))%>)</b></td>
		<td width="120"><%=appendEntity.APPEND_DATE%></td>
		<td width="24">
...
<% if(isWriteAuth && bbsEntity.BBS_USE_APPEND == 1){ %>
	<tr>
		...
		<td><input type="text" name="APPEND_CONTENT" id="APPEND_CONTENT" class="k_inpColor" style="width: 90%;" /> 
		<a href="javascript:board_detail_space.board_detail.appendReply();"><img src="/images/kor/btn/btnB_add.gif" /></a></td>
	</tr>
<% } %>
...
<script type="text/javascript" src="/js/kor/bbs/board_detail.js"></script>

<script language="javascript">

function iframeView(){
	var objForm = document.forms['f_board_detail<%=uniqStr %>'];
	 var sUserAgent = navigator.userAgent.toLowerCase();
    var isIE     = (sUserAgent.indexOf("msie")!=-1 && sUserAgent.indexOf("opera")==-1 && window.document.all) ? true:false;
	setTimeout( function(){
		if(isIE){
			try{
				document.getElementById('board_detail_iframe<%=uniqStr%>').contentWindow.document.getElementById('detail_iframe_content').innerHTML = objForm.temp_content.value;
			}catch(e){
				setTimeout(iframeView,50);
			}	
		}else{ 		
			document.getElementById('board_detail_iframe<%=uniqStr%>').document.getElementById('detail_iframe_content').innerHTML = objForm.temp_content.value;
		}},50);	
}
iframeView();
</script>