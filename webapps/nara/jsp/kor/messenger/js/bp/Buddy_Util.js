   ...
   var tmpStr;
    for(i = 0; i<len; i++)
    {
         ...
         if (buddyListArr[10] =='3')
       	 ...
       	 tmpStr =  '<font title="'+buddyListArr[3] + '(' + buddyListArr[1] + ')'+'">' 
       	 			+displayConcat( buddyListArr[3] + '(' + buddyListArr[1] + ')') 
       	 			+'</font>';
         ...
         var nodes = new Ext.tree.TreeNode
                     ({
                        ...
                        //text: buddyListArr[3] + '(' + buddyListArr[1] + ')', //name
                        text: tmpStr, //name
...
//   progress.destroy();
  ...
  // hidef.WatcherListReq(hidef.user_id, '3');
   ...
   window.setTimeout("hidef.WatcherListReq(hidef.user_id, '3');", 2500);
   window.setTimeout("hidef.AppleWorkRequest('30004102', hidef.user_id);", 3000);
...
function displayConcat(myName){
	 //문자열 자르기...........    
        ...
        {
			if(myName.charCodeAt(i)>128){
            	strCharCounter += 2;
            }	
			else{
             	strCharCounter++;
			}
		    	strChar+=myName.substring(i,i+1);
			  
			  if(strCharCounter> maxlength){
			    valuelength  =	totmaxlength - strCharCounter
				strChar += "......................".substring(0,valuelength);
			  	break;
			 }
		}
		//alert(strChar);
                   ...
                   handler : function()
                   {
                       var n = tree.getSelectionModel().getSelectedNode();
                        ...
                        if(n !=null)
                        {
                           ...
                           Note_popup = false;
                           js_Note_open(arr[7], arr[3] ,"2342423424234");
                        }
                   }
               },'-'/*,{
                        id:  'mail',
   ...
   if (myDisplayNameFlag == "N")
   {
      ...
      if (before != displayName)
      {
         ...
         if (displayName.length >=7)
            name = displayName.substring(0,7)+"..";
      ...
      for(var k=0; k<buddyLength; k++) 
      {
         ...
         if (id =="dispName")
            ...
            //buddyNode[k].setText(arr[3]);//이름
          		...
          		buddyNode[k].setText('<font title="'+tmp+'">' +displayConcat(tmp)+'</font>');
...
function setPanelXY(obj){
	
	var position = panel.getPosition();
	var height = panel.getFrameHeight();
	var width = panel.getFrameWidth();
    //document.all.mlAdd_displayName.style.left = position[0]+21;
    //document.all.mlAdd_displayName.style.top  = position[1]+29;
	if(obj != null){
	    //obj.style.left = position[0]+21;
	    //obj.style.top  = position[1]+50;
	    document.getElementById("messenger_loading").style.left= position[0]+21;
	    document.getElementById("messenger_loading").style.top  = position[1]+50;
	}
     //    mlAdd_displayName.style.display = "block";	
}
