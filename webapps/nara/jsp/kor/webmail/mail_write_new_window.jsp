...
function searchFormByThis(formName){
	try{
...
<script type="text/javascript" charset="utf-8" src="/js/kor/editor/htmleditorExtend_new_window.js"></script>
<script LANGUAGE="JavaScript">
...
function sendMail<%=uniqStr%>(){
	var objForm = searchFormByThis("form_mail_write");
...
function mailSendAction<%=uniqStr%>() {
	...
	if( objForm.M_SECURE_YN != null && objForm.M_SECURE_YN.checked== true){ //security mail
    	...
    	createSecureFile(objForm.secureMailKey.value,
						 objForm.m_content.value,
						 uploadObj.GetLocalFileList());
		
		if (MAIL.StoreSmimeFilePath == null)
		{
			alert("보안메일 라이브러리 업데이트가 필요합니다.");
			return;
		}
		
		if (!uploadObj.SetEncryptedFilePath(MAIL.StoreSmimeFilePath))
			return;
...
function saveTempMail(){
	var objForm ;
	objForm = searchFormByThis("form_mail_write");
	isSaveTempMail = false;
	objForm.m_content.value = Ext.getCmp("editor_m_content"+objForm.uniqStr.value).getValue();
	if(objForm.m_content.value.length == 0){
  		alert("임시저장함에 저장할 본문 내용이 없습니다. ");
...
<% } %>
	<tr>
		...
		<td>
		<div style="float:left;margin:1px 3px 0 0;vertical-align: middle;align:left">
		<% if (!ccBcc.equals("03")) { %>
...
<script language="javascript">
getHtmlEditorInit();
//function setM_TOFocus<%=uniqStr%>(){
//  var objForm = searchFormByThis("form_mail_write");
//  objForm.M_TO.focus();  
//}
//setTimeout('setM_TOFocus<%=uniqStr%>()', 0);
</script>