...
<jsp:useBean id="anaGroupEntity"
	class="com.nara.jdf.db.entity.AnaUserGroupEntity" scope="request" />
...
<form method=post name="f" action="usergroup.admin.do"><input
	type=hidden name='method' value='usergroup_modify'> <input
	type=hidden name='USER_GROUP_IDX'
	value='<%=usergroupentity.USER_GROUP_IDX%>'> <input type=hidden
	name='P_USER_GROUP_IDX' value='<%=usergroupentity.P_USER_GROUP_IDX%>'>
<input type=hidden name='USER_GROUP_FUNCTION' value=''> <input
...
<table width="100%" class="k_puTableB">
	<tr>
		...
		<td width="75%"><input name="USER_GROUP_DEF_NAME" value="현재 그룹"
	...
	<tr>
		<th scope="row">그룹명</th>
		<td><input name="USER_GROUP_NAME" type="text"
			value="<%=usergroupentity.USER_GROUP_NAME%>" id="textfield4"
	...
	<tr>
		...
		<%if(UsersService.isSystemAdmin(request)){%> <input type="text"
			name="USER_GROUP_VOLUME" onKeyDown="javascript:isNumber();" size="4"
			maxlength="5" value='<%=usergroupentity.USER_GROUP_VOLUME%>'
			style="width: 50px"> <%}else{%> <input type="hidden"
			name="USER_GROUP_VOLUME" onKeyDown="javascript:isNumber();"
	...
	<tr>
		...
		<td><input type="text" name="USER_GROUP_SMS_QUOTA" size="4"
			maxlength="10" value='<%=usergroupentity.USER_GROUP_SMS_QUOTA%>'
			style="width: 50px" /> 건</td>
	</tr>
	<%}%>
</table>
<%
...
<table width="100%" class="k_puTableB">
	<tr>
		...
		<td><label><input type="radio" name="SERVICE_YN"
			value="S" <%= anaGroupEntity.SERVICE_YN.equals("S") ? "checked":"" %>>사용허가
		</label>&nbsp;&nbsp; <label><input type="radio" name="SERVICE_YN"
			value="N" <%= anaGroupEntity.SERVICE_YN.equals("N") ? "checked":"" %>>사용정지</label>&nbsp;&nbsp;
		<label><input type="radio" name="SERVICE_YN" value="W"
			<%= anaGroupEntity.SERVICE_YN.equals("W") ? "checked":"" %>>사용대기</label></td>
	</tr>
	<tr>
		<th scope="row">파일유효 기간</th>
		<td><input type="text" name="USERGROUP_PERIOD"
			onKeyDown="javascript:isNumber();" size=6
			value="<%= new BigDecimal(anaGroupEntity.USERGROUP_PERIOD) %>"
			style="width: 50px" /> 일간 <label> <input type="radio"
			name="PERIOD_LIMIT_YN" value="Y" checked id="radio55"
			<%= anaGroupEntity.PERIOD_LIMIT_YN.equals("Y") ? "checked":"" %> />
		파일유효 기간 제한 적용됨</label> &nbsp; <label> <input type="radio"
			name="PERIOD_LIMIT_YN" value="N" id="radio66"
			<%= anaGroupEntity.PERIOD_LIMIT_YN.equals("N") ? "checked":"" %> />
		파일유효 기간 제한 적용안됨</label></td>
	</tr>
	<tr>
		<th scope="row">다운로드 횟수</th>
		<td><label><input type="text" name="DOWN_CNT" size=6
			onKeyDown="javascript:isNumber();"
			value="<%= new BigDecimal(anaGroupEntity.DOWN_CNT) %>"
			style="width: 50px" /> 회</label> &nbsp;&nbsp; <label> <input
			type="radio" name="DOWN_LIMIT_YN" value="Y" id="radio77"
			<%= anaGroupEntity.DOWN_LIMIT_YN.equals("Y") ? "checked":"" %> />
		다운로드 횟수 제한 적용됨</label> &nbsp; <label> <input type="radio"
			name="DOWN_LIMIT_YN" value="N" id="radio78"
			<%= anaGroupEntity.DOWN_LIMIT_YN.equals("N") ? "checked":"" %> />
	...
	<tr>
		...
		<td style="padding-left: 100px"><label> <input
			...
			<%= anaGroupEntity.EXPIRE_DEL_YN.equals("Y") ? "checked":"" %> />
		기간만료 삭제 적용됨</label> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <label><input
			type="radio" name="EXPIRE_DEL_YN" value="N" id="label99"
			<%= anaGroupEntity.EXPIRE_DEL_YN.equals("N") ? "checked":"" %> />
		기간만료 삭제 적용안됨</label></td>
	</tr>
	<tr>
		<th scope="row">사용자 사용량</th>
		<td><input type="text" onKeyDown="javascript:isNumber();"
			name=USERGROUP_MAXQUOTA size=6
			value='<%= new BigDecimal(anaGroupEntity.USERGROUP_MAXQUOTA) %>'
			style="width: 50px" /> MB</td>
	</tr>
</table>
<%}%>
</div>
...
<div class="k_puLayBott"><a href="javascript:;"
	...
	onClick="javascript:self.close();"><img
