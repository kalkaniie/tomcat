...
<script type="text/javascript"
	src="/js/kor/usergroup/admin_user_group.js"></script>
...
function registGroup() {
  ...
  window.open( link ,"pop","scroll=no,status=no,toolbar=no,width=651,height=591");
}
...
function modify(){			// 그룹정보변경
  ...
  if(getOnlyCheckNode()){		// checkbox only ckeck
	  ...
	  window.open( link ,"modify","status=no,toolbar=no,scroll=no,status=no,toolbar=no,width=651,height=591");
   }else{
...
<form name="f" action='usergroup.admin.do' method=post><input
	type=hidden name="method" value="usergroup_main"> <input
	type=hidden name="USER_GROUP_IDX"
	value="<%=userGroupEntity.USER_GROUP_IDX %>"> <input
	type=hidden name="M_TO" value=""> <input type=hidden
	name=receiveHpValue value=''> <input type=hidden
	name=receiveHpText value=''> <input type=hidden
	name="USER_GROUP_NAME" value=""> <input type=hidden name="str"
	value=""> <input type=hidden name="USER_GROUP_IDX_SELECT"
	value="">

<div class="k_puTit">
<h2 class="k_puTit_ico2">도메인관리자 <strong>조직도관리</strong></h2>
<hr />
</div>
<ul class="k_tip_ul">
	<li>일괄등록 된 사용자를 자동으로 그룹핑 해주며, 사용자 조직을 관리할 수 있습니다.</li>
</ul>
<table class="k_puAdmin2_box" style="width: 800px">
	<caption><b><%=domainEntity.DOMAIN_NAME%></b></caption>
	<tr>
		<th width="220" class="k_puAdmin2_boxTh">
		<div id="tree-div"
			style="overflow: auto; height: 650px; width: 220px; border: 1px solid #c3daf9;">
		</div>
		</th>
		...
		<div class="k_puAdmin_sBox"
			style="display: block; position: relative; margin: 10px 15px 0">
		<label> <input type="radio" name="strIndex" value="USERS_NAME"
			checked /> 이름</label>&nbsp;&nbsp;&nbsp; <label> <input type="radio"
			name="strIndex" value="USERS_ID" /> ID</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<!--
     <select>
      <option value="1">전체그룹</option>
              <option value="2">현재그룹</option>
     </select>--> <input type="text" style="width: 180px"
			class="k_intx00" name="strKeyword" /> <a href="javascript:search();"><img
			src="/images/kor/btn/popup_search.gif" /></a></div>
		</td>
	</tr>
</table>
<div class="k_adminBtn" style="padding-top: 10px"><a
	href='javascript:onClick=registGroup()'><img
	src="/images/kor/btn/popup_groupCreat.gif" /></a> <a
	href='javascript:onClick=modify()'><img
	src="/images/kor/btn/popup_groupInfoModify.gif" /></a> <a
	href='javascript:onClick=deleteGroup()'><img
	src="/images/kor/btn/popup_groupDele.gif" class="st_btn2" /></a> <a
	href="javascript:onClick=intranetGroup();"><img
	src="/images/kor/btn/popup_copyIntra.gif" /></a> <a
	href="javascript:onClick=selectPublic();"><img
	src="/images/kor/btn/popup_copyCommAdr.gif" /></a> <a
	href="javascript:onClick=sendmail();"><img
	src="/images/kor/btn/popup_mailWrite2.gif" /></a></div>
</form>
