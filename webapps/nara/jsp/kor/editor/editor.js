...
document.write('.btn     { width: 22px; height: 22px; border: 1px solid #E7EDEF; margin: 0; padding: 0; background-color: #f0efef;}\n');
document.write('.btnOver { width: 22px; height: 22px; border: 1px outset; background-color: #f0efef;}\n');
document.write('.btnDown { width: 22px; height: 22px; border: 1px inset; background-color: #F3F6F7; }\n');
document.write('.btnNA   { width: 22px; height: 22px; border: 1px solid #E7EDEF; filter: alpha(opacity=25); }\n');
...
this.width =  "auto";
this.height = "auto";
this.bodyStyle = 'background-color: #FFFFFF; font-family: "굴림"; font-size: 10pt; scrollbar-3dlight-color:#D5E4E4; scrollbar-arrow-color:#BED3D3; scrollbar-base-color:#FFFFFF; scrollbar-darkshadow-color:#FFFFFF; scrollbar-face-color:#FFFFFF; scrollbar-highlight-color:#FFFFFF; scrollbar-shadow-color:#D5E4E4; overflow:auto; border:1 solid #D5E4E4;';
this.PStyle = 'margin-top:0px;margin-bottom:0px;';
this.imgURL = _editor_url + '../../../images/tabmenu/';
this.debug  = 0;
...
//  ['linebreak'],
    ...
    ['HorizontalRule','Createlink','InsertImage','InsertTable','InsertChar','separator','htmlmode']];
...
this.btnList = {
    ...
    "bold":           ['Bold',                 'Bold',               'editor_action(this.id)',  'edt_bold.gif'],
    "italic":         ['Italic',               'Italic',             'editor_action(this.id)',  'edt_italic.gif'],
    "underline":      ['Underline',            'Underline',          'editor_action(this.id)',  'edt_underline.gif'],
    "strikethrough":  ['StrikeThrough',        'Strikethrough',      'editor_action(this.id)',  'edt_strike.gif'],
    "subscript":      ['SubScript',            'Subscript',          'editor_action(this.id)',  'edt_scriptSub.gif'],
    "superscript":    ['SuperScript',          'Superscript',        'editor_action(this.id)',  'edt_scriptSup.gif'],
    "justifyleft":    ['JustifyLeft',          'Justify Left',       'editor_action(this.id)',  'edt_alignLeft.gif'],
    "justifycenter":  ['JustifyCenter',        'Justify Center',     'editor_action(this.id)',  'edt_alignCenter.gif'],
    "justifyright":   ['JustifyRight',         'Justify Right',      'editor_action(this.id)',  'edt_alignRight.gif'],
    "orderedlist":    ['InsertOrderedList',    'Ordered List',       'editor_action(this.id)',  'edt_listNum.gif'],
    "unorderedlist":  ['InsertUnorderedList',  'Bulleted List',      'editor_action(this.id)',  'edt_listBullet.gif'],
    "outdent":        ['Outdent',              'Decrease Indent',    'editor_action(this.id)',  'edt_indentLess.gif'],
    "indent":         ['Indent',               'Increase Indent',    'editor_action(this.id)',  'edt_indentMore.gif'],
    "forecolor":      ['ForeColor',            'Font Color',         'editor_action(this.id)',  'edt_colorTxt.gif'],
    "backcolor":      ['BackColor',            'Background Color',   'editor_action(this.id)',  'edt_colorBg.gif'],
    "horizontalrule": ['InsertHorizontalRule', 'Horizontal Rule',    'editor_action(this.id)',  'edt_hr.gif'],
    "createlink":     ['CreateLink',           'Insert Web Link',    'editor_action(this.id)',  'edt_link.gif'],
    "insertimage":    ['InsertImage',          'Insert Image',       'editor_action(this.id)',  'edt_image.gif'],
    "inserttable":    ['InsertTable',          'Insert Table',       'editor_action(this.id)',  'edt_table.gif'],
    "insertchar":     ['InsertChar',           'symbol',        'editor_action(this.id)',  'edt_separator.gif'],
    "htmlmode":       ['HtmlMode',             'View HTML Source',   'editor_setmode(\''+objname+'\')', 'edt_html.gif'],
...
alert('editor_generate(objname) : objname = ' + objname);
	editor_generate(objname,userConfig) ;
}		
...
alert('editor_generate(objname) : objname = ' + objname);
  ...
  document.getElementById(objname).config = config;
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      // fontname
      if (btnName == "fontname") {
        toolbar += '<select id="_' +objname+ '_FontName" onChange="editor_action(this.id)" unselectable="on" class="fos5" >';
      ...
      // fontsize
      if (btnName == "fontsize") {
        toolbar += '<select id="_' +objname+ '_FontSize" onChange="editor_action(this.id)" unselectable="on" class="fos5">';
        for (var fontsize in config.fontsizes) {
          if(config.fontsizes[fontsize] == 2)
            toolbar += '<option value="' +config.fontsizes[fontsize]+ '" selected>' +fontsize+ '</option>'
          else
      ...
      if (btnName == "fontstyle") {
        toolbar += '<select id="_' +objname+ '_FontStyle" onChange="editor_action(this.id)" unselectable="on" class="fos5">';
      ...
      if (btnName == "separator") {
        toolbar += '<img src="'+_editor_url + '../../../images/line/lineSection_tw.gif"/>';
        continue;
      }
      // buttons
      var btnObj = config.btnList[btnName];
      if (btnName == 'linebreak') { alert("htmlArea error: 'linebreak' must be in a subcategory by itself, not with other buttons.\n\nhtmlArea wysiwyg editor not created."); return; }
  ...
  // build editor
  var editor = '<span id="_editor_toolbar"><table border=0 cellspacing=0 cellpadding=0 bgcolor="#f0efef" style="padding: 1 0 0 2" width=' + config.width + ' unselectable="on"><tr><td>\n'
  + toolbar
  + '</td></tr></table>\n'
  + '</td></tr></table></span>\n'
  ...
  if( navigator.userAgent.indexOf('MSIE') !=-1){
    ...
    var parsedHTML = r.createContextualFragment(editor);
    el.parentNode.insertBefore(parsedHTML,el);

  }
  

//  objname.insertAdjacentHTML('afterEnd', editor)
...
function editor_action(button_id) {
  ...
  var button_obj = document.getElementById(button_id);// document.all[button_id];
  var editor_obj = document.getElementById("_" +objname + "_editor");//document.all["_" +objname + "_editor"];
...
//alert(editor_obj+"||"+editor_obj.tagName.toLowerCase());
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert table
      ...
      showModalDialog(_editor_url + "popups/insert_table.html?"+objname,
		...
		else if (cmdID.toLowerCase() == 'insertchar'){
      showModalDialog(_editor_url + "popups/insert_charset.jsp?"+objname,
                                 window,
                                 "dialogWidth:400px; dialogHeight:460px;resizable: yes; help: no; status: no; scroll: no; ");
    }
...
function editor_event(objname,runDelay) {
  ...
  var config = document.getElementById(objname).config;
  var editor_obj  = document.getElementById("_" +objname+  "_editor");//document.all["_" +objname+  "_editor"];       // html editor object
...
function editor_updateToolbar(objname,action) {
  ...
  if (action == "enable" || action == "disable") {
    ...
    for (var idxN in tbItems) {
      ...
      var tbObj = document.getElementById("_" +objname+ "_" +tbItems[idxN]);//document.all["_" +objname+ "_" +tbItems[idxN]];
  ...
  var fontname_obj = document.getElementById("_" +objname+ "_FontName");// document.all["_" +objname+ "_FontName"];
  if (fontname_obj) {
  ...
  var fontsize_obj = document.getElementById("_" +objname+ "_FontSize");// document.all["_" +objname+ "_FontSize"];
  if (fontsize_obj) {
  ...
  var classname_obj = document.getElementById("_" +objname+ "_FontStyle");// document.all["_" +objname+ "_FontStyle"];
  if (classname_obj) {
  ...
  for (i=0; i<IDList.length; i++) {
    var btnObj = document.getElementById("_" +objname+ "_" +IDList[i]);// document.all["_" +objname+ "_" +IDList[i]];
...
function init() {  
	...
	//if (_timer) clearInterval(_timer);  // do stuff
};
...
function editor_setmode(objname, mode) {
  ...
  // wait until document is fully loaded
	if( navigator.userAgent.indexOf('MSIE') ==-1){ 
	  if (document.addEventListener) {  
	  	//document.addEventListener("DOMContentLoaded", editor_setmode(objname,mode), false);
	  }
  	//window.onload = init;
  }else{
  ...
  //편지모드를 중시시킬경우에 사용
  //var TextEdit   = '<textarea ID="_' +objname + '_editor" style="width:' +editor_obj.style.width+ '; height:' +editor_obj.style.height+ '; margin-top: -1px; margin-bottom: -1px; scrollbar-3dlight-color:#D5E4E4; scrollbar-arrow-color:#BED3D3; scrollbar-base-color:#FFFFFF; scrollbar-darkshadow-color:#FFFFFF; scrollbar-face-color:#FFFFFF; scrollbar-highlight-color:#FFFFFF; scrollbar-shadow-color:#D5E4E4; overflow:auto; border:1 solid #D5E4E4;" onfocus=blur()></textarea>';
  var TextEdit   = '<textarea ID="_' +objname + '_editor" style="width:' +editor_obj.style.width+ '; height:' +editor_obj.style.height+ '; margin-top: -1px; margin-bottom: -1px; scrollbar-3dlight-color:#D5E4E4; scrollbar-arrow-color:#BED3D3; scrollbar-base-color:#FFFFFF; scrollbar-darkshadow-color:#FFFFFF; scrollbar-face-color:#FFFFFF; scrollbar-highlight-color:#FFFFFF; scrollbar-shadow-color:#D5E4E4; overflow:auto; border:1 solid #D5E4E4;"></textarea>';
  var RichEdit   = '<iframe ID="_' +objname+ '_editor"    style="width:' +editor_obj.style.width+ '; height:' +editor_obj.style.height+ '; " onfocus=blur() tabindex=3></iframe>';
 ...
 // src="' +_editor_url+ 'popups/blank.html"
  ...
  //
  //alert(editor_obj.tagName.toLowerCase());
  if (mode == "textedit" || editor_obj.tagName.toLowerCase() == 'iframe') {
    ...
    editor_obj = document.getElementById("_" +objname + "_editor");// document.all["_" +objname + "_editor"];
  ...
  //
    ...
    html += '</style>\n'
      + '</head>\n'
      + '<body contenteditable="true" topmargin=2 leftmargin=2'
...
function editor_insertHTML(objname, str1, str2, reqSel) {
  var config     = document.getElementById(objname).config;
  var editor_obj = document.getElementById("_" +objname + "_editor");// document.all["_" +objname + "_editor"];    // editor object
  if (str1 == null) { str1 = ''; }
  if (str2 == null) { str2 = ''; }
