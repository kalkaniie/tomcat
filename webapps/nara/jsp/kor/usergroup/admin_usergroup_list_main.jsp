...
function namecard(USERS_IDX){
    ...
    window.open( link ,"NameCard","status=no,toolbar=no,scrollbars=yes,width=500,height=450");
}
...
<script type="text/javascript" src="/js/kor/usergroup/admin_user_group_list.js"></script>

<form method=post name="f" action="usergrouplist.admin.do">
<input type=hidden name='method' value='usergroup_list_main'> 
<input type=hidden name='USER_GROUP_IDX' value='<%=USER_GROUP_IDX%>'>
<input type=hidden name='USER_GROUP_IDX_SELECT' value='0'> 
...
<input type=hidden name='strKeyword'value''>
<input type=hidden name='strType'value''> 
<input type=hidden name='ADDRESS_GROUP_STMT' value=''> 
<input type=hidden name='ADDRESS_GROUP_IDX' value=''> 
<input type=hidden name='nMode' value=''> 
<!-- 공유주소록으로 복사 기능에 대한 변수 설정 --> 
<input type=hidden name='PUBLICGROUP_IDX' value=''> 
<!-- 사용자 정보를 넘긴다. -->
