...
<input type=hidden name='method' value='usergroup_insert'>
<input type=hidden name='P_USER_GROUP_IDX' value=''>
<input type=hidden name='USER_GROUP_SORT_NO' value='<%=usergroupentity.USER_GROUP_SORT_NO%>'>
...
<div class="k_puLayout">
    ...
    <div class="k_puLayContIn">
      ...
      <table width="100%" class="k_puTableB">
        <tr>
          ...
          <td width="75%"><input name="USER_GROUP_DEF_NAME" value="" readonly type="text" id="textfield3" style="width:200px"/>
        <a href = "javascript:select();"><img src="/images/kor/btn/popupSm_find.gif"/></a></td>
        </tr>
        <tr>
          <th scope="row">그룹명</th>
          <td><input name="USER_GROUP_NAME" type="text" id="textfield4" style="width:200px"/></td>
        </tr>
        <tr>
          <th scope="row">가입시 기본 그룹</th>
          <td><label><input type="checkbox" name="USER_GROUP_DEFAULT" value="1" >
              사용자 가입시 현재그룹에 속함</label>
          </td>
        </tr>
        <tr>
          ...
          <td>
          <%if(UsersService.isSystemAdmin(request)){%>
            <input type="text" name="USER_GROUP_VOLUME" size="4"  maxlength="5"  onKeyDown="javascript:isNumber();" value='<%=usergroupentity.USER_GROUP_VOLUME%>' style="width:50px">
              <%}else{%>
        ...
        <tr>
          ...
          <td><ul class="k_puList">
               ...
               <% if(DomainService.isValidModule(request,"pop")){ %>
                ...
                <script>document.write(function_text[1]);</script>
                </label>
                </li>
                <%}%>
               <% if(DomainService.isValidModule(request,"imap")){ %>
                <li>
                <label><input type="checkbox" name="USER_GROUP_FUNCTION_3" value="1">
                <script>document.write(function_text[2]);</script>
                </label>
                </li>
                <%}%>
               <% if(DomainService.isValidModule(request,"schedule")){ %>
                <li>
                <label><input type="checkbox" name="USER_GROUP_FUNCTION_4" value="1">
                <script>document.write(function_text[3]);</script>
                </label>
                </li>
                <%}%>
               <% if(DomainService.isValidModule(request,"file")){ %>
                <li>
                <label><input type="checkbox" name="USER_GROUP_FUNCTION_5" value="1">
                <script>document.write(function_text[4]);</script>
                </label>
                </li>
                <%}%>
                <% if(DomainService.isValidModule(request,"bbs")){ %>
                <li>
                <label><input type="checkbox" name="USER_GROUP_FUNCTION_6" value="1">
                <script>document.write(function_text[5]);</script>
                </label>
                </li>
                <%}%>
                <% if(DomainService.isValidModule(request,"intranet")){ %>
                <li>
                <label><input type="checkbox" name="USER_GROUP_FUNCTION_7" value="1">
                <script>document.write(function_text[6]);</script>
                </label>
                </li>
                <%}%>
                <% if(DomainService.isValidModule(request,"sms")){ %>
                <li>
                <label><input type="checkbox" name="USER_GROUP_FUNCTION_8" value="1">
                <script>document.write(function_text[7]);</script>
                </label>
                </li>
                <%}%>
                <% if(DomainService.isValidModule(request,"archive")){ %>
                <li>
                <label><input type="checkbox" name="USER_GROUP_FUNCTION_9" value="1">
                <script>document.write(function_text[8]);</script>
                </label>
                </li>
                <%}%>
                <% if(DomainService.isValidModule(request,"massenger")){ %>
                <li>
                <label><input type="checkbox" name="USER_GROUP_FUNCTION_10" value="1">
                <script>document.write(function_text[9]);</script>
                </label>
                </li>
                <%}%>
                <% if(DomainService.isValidModule(request,"webfile")){ %>
                <li>
                <label><input type="checkbox" name="USER_GROUP_FUNCTION_11" value="1">
                <script>document.write(function_text[10]);</script>
                </label>
                </li>
                <%}%>
                <% if(DomainService.isValidModule(request,"note")){ %>
                <li>
                <label><input type="checkbox" name="USER_GROUP_FUNCTION_12" value="1">
                <script>document.write(function_text[11]);</script>
				</label>
				<li>
				<%}%>
          </ul></td>
     ...
     <% if(DomainService.isValidModule(request,"webmail")){%>
        <tr>
          ...
          <td>
          <label><input type=radio name="USER_GROUP_MAIL" value="1">
              있음</label>
            &nbsp;&nbsp;&nbsp;
            <label><input type=radio name="USER_GROUP_MAIL" value="0" checked>
            없음</label></td>
        </tr>
    <%}%>
    <% if(DomainService.isValidModule(request,"schedule")){%>
        <tr>
          ...
          <td><label><input type=radio name="USER_GROUP_SCHEDULE" value="1">
              있음</label>
            &nbsp;&nbsp;&nbsp;
            <label><input type=radio name="USER_GROUP_SCHEDULE" value="0" checked>
            없음</label></td>
        </tr>
    <%}%>
        <tr>
          ...
          <td>
          <label><input type=radio name="USER_GROUP_ADDRESS" value="1">
             있음</label>
            &nbsp;&nbsp;&nbsp;
            <label><input type=radio name="USER_GROUP_ADDRESS" value="0" checked >
            없음</label></td>
        </tr>
        <tr>
          <th scope="row">조직도 사용권한</th>
          <td>
          <label><input type=radio name="USER_GROUP_STRUCTURE" value="1">
              있음</label>
            &nbsp;&nbsp;&nbsp;
            <label><input type=radio name="USER_GROUP_STRUCTURE" value="0" checked>
            없음</label></td>
        </tr>
     <% if(DomainService.isValidModule(request,"sms")){%>
        <tr>
          <th scope="row">SMS 사용량</th>
          <td><input type="text" name="USER_GROUP_SMS_QUOTA" onKeyDown="javascript:isNumber();" size="4" maxlength="10" value='<%=usergroupentity.USER_GROUP_SMS_QUOTA%>' style="width:50px"/>
            건</td>
        </tr>
     <%}%>
      </table>
  ...
  %>
      ...
      <table width="100%" class="k_puTableB">
        <tr>
          <th width="25%" scope="row">서비스상태</th>
          <td><label>
            <input type="radio" name="SERVICE_YN" value="S" checked>
            사용허가 </label>
            &nbsp;&nbsp;
            <label>
              <input type="radio" name="SERVICE_YN" value="N">
              사용정지</label>
            &nbsp;&nbsp;
            <label>
              </td>
        ...
        <tr>
          ...
          <td><input type="text" name="USERGROUP_PERIOD" onKeyDown="javascript:isNumber();" size=6 value="<%= conf.getString("com.nara.kebimail.anaconda.usersperiod")%>" style="width:50px"/>
            일간
            <label>
    <input type="radio" name="PERIOD_LIMIT_YN" value="Y" checked id="radio55" />
              파일유효 기간 제한 적용됨</label>
  &nbsp;
  <label>
  <input type="radio" name="PERIOD_LIMIT_YN" value="N" id="radio66"/>
    파일유효 기간 제한 적용안됨</label></td>
        </tr>
        <tr>
          <th scope="row">다운로드 횟수</th>
          <td><input type="text" name="DOWN_CNT" onKeyDown="javascript:isNumber();" size=6 value="<%= conf.getString("com.nara.kebimail.anaconda.downloadcnt")%>" style="width:50px"/>
            회
            <label>&nbsp;&nbsp;
    <input type="radio" name="DOWN_LIMIT_YN" value="Y" checked id="radio77" />
              다운로드 횟수 제한 적용됨</label>
  &nbsp;
  <label>
  <input type="radio" name="DOWN_LIMIT_YN" value="N" id="radio78" />
    다운로드 횟수 제한 적용안됨</label></td>
        </tr>
        <tr>
          <th scope="row">다운로드 만료기간</th>
          <td style="padding-left:100px">
          <label>
            <input type="radio" name="EXPIRE_DEL_YN" value="Y" checked id="label88" />
            기간만료 삭제 적용됨</label>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  <label><input type="radio" name="EXPIRE_DEL_YN" value="N" id="label99" />
  기간만료 삭제 적용안됨</label></td>
        </tr>
        <tr>
          <th scope="row">사용자 사용량</th>
          <td><input type="text" name=USERGROUP_MAXQUOTA size=6 onKeyDown="javascript:isNumber();" value='<%= conf.getString("com.nara.kebimail.anaconda.diskquota")%>' style="width:50px;ime-mode:disabled"/>
            MB</td>
        </tr>
      </table>
   <%}%>
  ...
  <div class="k_puLayBott">
    <a href="javascript:chkSubmit();"><img src="/images/kor/btn/btnA_save.gif"/></a>
    <a href="javascript:window.close();"><img src="/images/kor/btn/btnA_cancel.gif"/></a>
  </div>
</div>
<script language=javascript>
<%
