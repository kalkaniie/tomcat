...
function addAddrGrpByUserGroup(){
	var allCheckedNode = usergroup_address_space.main.getTreeCheckedNode();
	...
	var link = "address.auth.do?method=getAddressGrpTreeView&REG_TYPE=GROUP&USER_GROUP_IDX="+USER_GROUP_IDX+"&USER_GROUP_NAME="+USER_GROUP_NAME;
  	...
  	newWindowOpen('주소록추가',320,400,link);
}
function addAddressByUserGroupList(){
	var rows = usergroup_address_space.main.getGridSelectionModel().getSelections();
	...
	if( rows.length == 0){
        ...
        for( i=0; i<rows.length; i++){
        	USER_GROUP_LIST_IDXS += (rows[i].data.USER_GROUP_LIST_IDX + ",");
      	}	
      	
      	USER_GROUP_LIST_IDXS = USER_GROUP_LIST_IDXS.substring(0, USER_GROUP_LIST_IDXS.length-1);
      	
	  	var link = "address.auth.do?method=getAddressGrpTreeView&REG_TYPE=GROUP_LIST&USER_GROUP_LIST_IDXS="+USER_GROUP_LIST_IDXS;
	  	//var link = encodeURI("address.auth.do?method=getAddressGrpTreeView&REG_TYPE=GROUP_LIST&USER_GROUP_LIST_IDXS="+USER_GROUP_LIST_IDXS);
	  	newWindowOpen('주소록추가',320,400,link);
	}
}
</script>
<form name="user_group_public_form" action='' method=post>
...
<% if(UsersService.IsAuthOfGroupAddress(request)){ %><a href="javascript:goGrpPage('publicgroup')">공용 주소록</a><%}%>
  <% if(UsersService.IsAuthOfGroupStructure(request)){ %><a href="javascript:goGrpPage('usergroup')"><b>조직도</b></a><%}%>
</p>
<p class="k_fltR"><b>조직도 찾기</b> <select name="strIndex">
	<option value="USERS_NAME">이름</option>
	<option value="USERS_ID">ID</option>
</select> 
<input name="strKeyword" type="text" value="" class="k_inpColor" style="width: 75px" onkeydown="javascript:if(event.keyCode == 13) { addr_usergroup_search(); return false}"/> 
<a href="javascript:addr_usergroup_search();"><img src="/images/kor/ico/btn_find.gif" /></a></p>
</div>

<div class="k_gridA3">
<a href="javascript:;" onClick="javascript:addr_user_group_sendmail();"><img src="/images/kor/btn/btnA_mailWrite.gif" /></a> 
<a href="javascript:;" onClick="javascript:addAddrGrpByUserGroup();"><img src="/images/kor/btn/btnA_userGAdrsAdd.gif" /></a>
<a href="javascript:;" onClick="javascript:addAddressByUserGroupList();"><img src="/images/kor/btn/btnA_userAdrsAdd.gif" /></a>
<%-- if(UsersService.isValidModule(request,"sms")){ %>
<a href="javascript:onclick=addr_user_group_sendSms();"><img src="/images/kor/btn/btnA_sendSms.gif" /></a> <%} --%>
</div>
...
<table class="k_treNlis">
	<tr>
		...
		<div class="k_treBox_tit">조직도</div>
