...
<script type="text/javascript" src="/js/kor/address/address_all_pop.js"></script>
...
function sendAction(type){
	...
	for( i=0; i<rows.length; i++){
       	...
       	if( type == 'mail')
       		...
       		USERS_LIST.push(rows[i].data.USERS_EMAIL);
       	else if( type == 'address'){
       		 USERS_LIST.push(rows[i].data.USERS_EMAIL);
       		 USERS_NAME.push(rows[i].data.USERS_NAME);
       		}
       	is_flag = true;	
	}
    if(!is_flag){
    ...
    <% if( UserSession.getString(request, "USERS_LOGIN_MODE") != null&& UserSession.getString(request, "USERS_LOGIN_MODE").equals("std")){ %>
	    if( type == "mail"){
	 		opener.goRightDivRender("webmail.auth.do?method=mail_write&M_TO=" +users_id,"편지쓰기");
	 		self.close();
	 	}else if( type == 'note'){
	  		opener.goRightDivRender('note.auth.do?method=showMain_std&NOTE_ISREAD=N','note');
	  		MM_openBrWindow("/mail/note.auth.do?method=note_regist_form_search&USERS_LIST=" + USERS_LIST, "_self","");
	  	}else if( type == 'address'){
	  		popupaddressAdd(USERS_LIST , USERS_NAME);
	  	}
    <%}else{%>
	    if( type == 'mail'){
	 		opener.goRightDivRenderParams("webmail.auth.do", "method=mail_write&M_TO=" +users_id, "편지쓰기:사용자검색-" + getUniqueString());
	 		self.close();
	 	}else if( type == 'note'){
	  		opener.goRightDivRenderParams('note.auth.do?method=showMain&NOTE_ISREAD=N','note' ,'쪽지');
	  		MM_openBrWindow("/mail/note.auth.do?method=note_regist_form_search&USERS_LIST=" + USERS_LIST, "_self","");
	  	}else if( type == 'address'){
	  		popupaddressAdd(USERS_LIST , USERS_NAME);
	  	}
...
<table class="h2">
	<tr>
		<td><img src="/images_std/kor/bullet/arrow_pop.gif" align="top" />사용자 검색</td>
	</tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-bottom:4px;">
  <tr>
    <td class="btn_bgtd_c" style="border-top:none; background:none;"><strong style="font-size:12px; vertical-align:middle;">검색</strong>　<select name="strIndex" id="select">
